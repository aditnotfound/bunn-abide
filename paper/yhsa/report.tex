...
\hypersetup{
  ...
  pdfsubject={Current-status research report},
...
\fancyhead[R]{\small Current-status report}
\fancyfoot[C]{\thepage}
...
\newcommand{\draftnote}{\textbf{Current-status draft.} This version reports the completed and audited ABIDE-I operator study only. Planned post-hoc mechanism experiments and external validation are not presented as completed results.}
...
\hline
Report Version & Current-status evidence report, 13 August 2026 \\
\hline
\end{tabularx}
...
detected predictive or representation-preservation advantage. The study does
not establish a biological bundle geometry, a clinical diagnostic system, or
a universal result for BuNNs. Its contribution is a controlled audit of a
propagation operator in a dense, globally informed connectome setting.
...
propagation averages those profiles. Learned orthogonal transport changes how
features are aligned, but the matched controls show no retained predictive
benefit. This interpretation is consistent with the observed curves. It is
not yet a direct causal proof of which internal feature components were lost.
...
mediation analysis. Finally, the completed report does not contain an external
ABIDE-II validation cohort.

\subsection{Planned extensions and their evidential role}

The next report version should add only experiments that survive independent
integrity checks. The highest-priority extension uses the accepted Study 1
checkpoints to ask more directly what changes when propagation is removed,
replaced, or transported. That analysis can test candidate explanations for
the density curves without retraining thousands of models. Deeper matched
GCN/BuNN networks, a leakage-free Wang-style time-series model, synthetic
ground-truth geometry, and prospective ABIDE-II evaluation are separate
studies. They should not be blended into the confirmatory result or described
as completed until their protocols and outputs are frozen and audited.
...
capacity and runtime without a detected predictive benefit. The negative
result is informative because it was obtained through matched controls,
held-out-site evaluation, strong non-graph baselines, gauge-aware diagnostics,
and a frozen decision rule.

\clearpage
\section*{Current Evidence and Remaining Work}
\addcontentsline{toc}{section}{Current Evidence and Remaining Work}
...
Mechanism evidence & Common-frame representation curves, matched anchors, site influence, seed stability, and efficiency. & Checkpoint perturbations, component-level feature-loss analysis, ground-truth synthetic geometry. \\
