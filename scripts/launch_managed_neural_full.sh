#!/usr/bin/env bash
# Launch the frozen neural smoke/full runner detached from SSH with SNS alerts.
set -Eeuo pipefail

if [[ $# -lt 2 ]]; then
  echo "Usage: $0 RUN_ID RUN_KIND [run_neural_full.py arguments...]" >&2
  exit 64
fi
...
CONTROL_DIR="$PROJECT_DIR/outputs/runs/neural-full/.control/$RUN_ID"
...
for REQUIRED_PATH in \
  ...
  if [[ ! -r "$REQUIRED_PATH" ]]; then
    echo "Missing required full-run input: $REQUIRED_PATH" >&2
    exit 78
  fi
done
...
(
  ...
  exec "$PROJECT_DIR/.venv/bin/python" "$PROJECT_DIR/scripts/run_neural_full.py" \
